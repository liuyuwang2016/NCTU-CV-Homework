...
//Mat KD_Tree(Mat puzzleDescriptor, Mat sampleDescriptor){
//
//}
Mat imgCutter(Mat ImgOri, int iniX, int finX, int iniY, int finY) {
	Mat ImageCut = Mat(ImgOri.rows, ImgOri.cols, CV_8UC3);
	for (int rowIndex = 0; rowIndex <ImgOri.rows; rowIndex++) {
		for (int colIndex = 0; colIndex <ImgOri.cols; colIndex++) {
			if ((colIndex >= iniX && colIndex <= finX) && (rowIndex >= iniY && rowIndex <= finY))
				ImageCut.at<Vec3b>(rowIndex, colIndex) = ImgOri.at<Vec3b>(rowIndex, colIndex);
		}
	}
	return ImageCut;
}

Mat affineMappingMatrix(Mat puzzleDescriptor, vector<KeyPoint> puzzleKeypoints, vector<KeyPoint> sampleKeypoints, Mat KNNP, int iteration)
{
	...
	/*When iteration = 10000, the result is very good for the table, the time is 210s;
	When iteration = 5000, the result is very good for the table too, the time is 143s;
	When iteration = 3500, the result is good enough for the table, the time is 129s;
	when iteration = 2000, the result is good for the first 5 puzzles, but not good for the next 2, the time is 107s;*/
...
void NCTUbackwardWarping(Mat puzzle, Mat& sample, Mat H)
{
	...
	for (int i = 0; i < sample.rows; i++) 
	{
		for (int j = 0; j < sample.cols; j++) 
		{
			...
			if (0 <= fy && fy < puzzle.rows && 0 <= fx && fx < puzzle.cols)
			{
				if (puzzle.at<Vec3b>(fy, fx)[0] != 0 && puzzle.at<Vec3b>(fy, fx)[1] != 0 && puzzle.at<Vec3b>(fy, fx)[2] != 0)
				{

					sample.at<Vec3b>(i, j)[0] = puzzle.at<Vec3b>(fy, fx)[0]; //g
					sample.at<Vec3b>(i, j)[1] = puzzle.at<Vec3b>(fy, fx)[1]; //r
					sample.at<Vec3b>(i, j)[2] = puzzle.at<Vec3b>(fy, fx)[2]; //b
				}
			}
		}
	}
...
int main() {
	...
	Mat logoPuzzle1 = imgCutter(puzzle1, 270, 390, 65, 468);
	imwrite("test/logo/logoPuzzle1.bmp", logoPuzzle1);
	Mat puzzle1Descriptor = siftDescriptorGet("test/logo/logoPuzzle1.bmp");
	vector<KeyPoint> puzzle1Keypoints = siftKeypoint("test/logo/logoPuzzle1.bmp");
	...
	Mat logoPuzzle2 = imgCutter(puzzle2, 250, 318, 134, 300);
	imwrite("test/logo/logoPuzzle2.bmp", logoPuzzle2);
	Mat puzzle2Descriptor = siftDescriptorGet("test/logo/logoPuzzle2.bmp");
	vector<KeyPoint> puzzle2Keypoints = siftKeypoint("test/logo/logoPuzzle2.bmp");
	...
	Mat logoPuzzle3 = imgCutter(puzzle3, 310, 413, 239, 365);
	imwrite("test/logo/logoPuzzle3.bmp", logoPuzzle3);
	Mat puzzle3Descriptor = siftDescriptorGet("test/logo/logoPuzzle3.bmp");
	vector<KeyPoint> puzzle3Keypoints = siftKeypoint("test/logo/logoPuzzle3.bmp");
	...
	//Mat logoPuzzle4 = imgCutter(puzzle4, 40, 235, 260, 480);
	//imwrite("test/logo/Puzzle4.bmp", puzzle4);
	...
	Mat logoPuzzleSample = imgCutter(sample, 182, 312, 87, 205);
	imwrite("test/logo/logoPuzzleSample.bmp", logoPuzzleSample);
	Mat sampleDescriptor = siftDescriptorGet("test/logo/logoPuzzleSample.bmp");
	vector<KeyPoint> sampleKeypoints = siftKeypoint("test/logo/logoPuzzleSample.bmp");
	...
	Mat knnVector1 = KNN(puzzle1Descriptor, sampleDescriptor);
	Mat affine1 = affineMappingMatrix(puzzle1Descriptor, puzzle1Keypoints, sampleKeypoints, knnVector1, 50000);
	Mat knnVector2 = KNN(puzzle2Descriptor, sampleDescriptor);
	Mat affine2 = affineMappingMatrix(puzzle2Descriptor, puzzle2Keypoints, sampleKeypoints, knnVector2, 50000);
	Mat knnVector3 = KNN(puzzle3Descriptor, sampleDescriptor);
	Mat affine3 = affineMappingMatrix(puzzle3Descriptor, puzzle3Keypoints, sampleKeypoints, knnVector3, 250000);
	Mat knnVector4 = KNN(puzzle4Descriptor, sampleDescriptor);
	Mat affine4 = affineMappingMatrix(puzzle4Descriptor, puzzle4Keypoints, sampleKeypoints, knnVector4, 50000);

	Mat knnVectorTarget = KNN(sampleDescriptor, targetDescriptor);
	Mat affineTarget = affineMappingMatrix(sampleDescriptor, sampleKeypoints, targetKeypoints, knnVectorTarget, 500000);
	
	Mat imageClone = target.clone();

	Mat H1 = affineTarget * affine1;
	Mat H2 = affineTarget * affine2;
	Mat H3 = affineTarget * affine3;
	Mat H4 = affineTarget * affine4;

	NCTUbackwardWarping(puzzle1, imageClone, H1);
	NCTUbackwardWarping(puzzle2, imageClone, H2);
	NCTUbackwardWarping(puzzle3, imageClone, H3);
	NCTUbackwardWarping(puzzle4, imageClone, H4);

	imwrite("NCTU_Logo_Warping.bmp", imageClone);
	imshow("NCTU Logo Warping", imageClone);
	/*imwrite("NCTU_Logo_Warping.bmp", imageClone);
	imshow("NCTU Logo Warping", imageClone);*/
